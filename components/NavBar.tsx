...
const LOGO_URL = "https://res.cloudinary.com/dhvwthnzq/image/upload/v1734355553/Transparent%20Governance%20Foundation/IMG_8356_xtbhut.png";
...
const DonateButton = memo(({ className = "" }: { className?: string }) => (
  <Link href="/donate">
    <button className={`bg-green-800 text-white px-4 py-2 hover:bg-green-900 transition duration-300 ease-in-out ${className}`}>
      Donate
    </button>
  </Link>
));
      ...
      {isMenuOpen && (
        <div className="lg:hidden px-4 pt-4 pb-6 bg-white">
          <ul className="space-y-10 mt-4">
            {NAV_LINKS.map((link) => (
              <NavLink key={link.href} {...link} pathname={pathname} onClick={toggleMenu} />
            ))}
            <li>
